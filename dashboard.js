...
// استخراج برند از هر جایی در عنوان (حساس به ترتیب و اولویت)
function extractBrandFromTitle(title) {
  if (!title || typeof title !== 'string') return 'نامشخص';

  const lowerTitle = title.toLowerCase();

  // جستجوی دقیق برندها در عنوان (اولین تطابق اولویت دارد)
...
function extractSizeAndTech(title) {
  ...
  if (lower.includes('qled') || lower.includes('کیوالایدی') || lower.includes('q led')) {
    tech = 'QLED';
  } else if (lower.includes('oled') || lower.includes('اولد')) {
    tech = 'OLED';
  }
  ...
  if (source === 'torob') {
    processed = raw.map((item, index) => {
      try {
        ...
        // اگر قیمت نداشت یا نام محصول نداشت → رد کن
        if (price_num <= 0 || !title.trim()) return null;
...
// بقیه توابع بدون تغییر اساسی (updateStats, updateUI, renderTable و ...)
...
function updateUI() {
  ...
  if (currentTab === 'torob') {
    ...
    // دیجی‌کالا - برندهای استخراج‌شده یا خالی
...
// بقیه توابع (renderTable, changePage, getFilteredData, sortTable, applyFilters, updateChart) بدون تغییر اساسی
